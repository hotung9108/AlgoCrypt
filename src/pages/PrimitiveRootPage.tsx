...
import { isPrimitiveRoot } from "@/algorithms/primitiveRoot";
import type { PrimitiveRootCheckStep } from "@/algorithms/primitiveRoot";

const PrimitiveRootPage: React.FC = () => {
    const [a, setA] = useState(5);
    const [n, setN] = useState(463);
    
    const [isPrimitive, setIsPrimitive] = useState<boolean | null>(null);
    const [phi, setPhi] = useState<number | null>(null);
    const [primeFactors, setPrimeFactors] = useState<number[]>([]);
    const [steps, setSteps] = useState<string[]>([]);
    const [checkSteps, setCheckSteps] = useState<PrimitiveRootCheckStep[]>([]);
    const [expandSteps, setExpandSteps] = useState(true);

    useEffect(() => {
        const calculate = () => {
            const { isPrimitive, phi, primeFactors, steps, checkSteps } = isPrimitiveRoot(a, n);
                        ...
                        <div className="lg:col-span-7 space-y-4">
                            ...
                            <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 shadow-lg overflow-hidden">
                                ...
                                <div className="bg-slate-950/60 rounded-lg p-3 font-mono text-xs space-y-2 max-h-[250px] overflow-y-auto">
                                    {checkSteps.length === 0 ? (
                                        <span className="text-slate-400">Enter values to see check details</span>
                                    ) : (
                                        checkSteps.map((step, idx) => (
                                            <div key={idx} className="flex items-center justify-between bg-slate-800/40 p-2 rounded border border-slate-700/50 hover:border-slate-600">
                                                ...
                                                <span className={`font-bold ${step.isPassed ? 'text-green-300' : 'text-red-300'}`}>
                                                    {a}^{step.quotient} mod {n} = {step.result} {step.isPassed ? "✓" : "✗"}
                                                </span>
                                            </div>
                                        ))
                                    )}
                            ...
                            <div className="bg-gradient-to-br from-blue-900/40 to-cyan-900/40 border border-blue-500/20 rounded-2xl p-6 shadow-xl flex flex-col h-fit">
                                ...
                                <div className="bg-slate-950/80 border border-white/5 rounded-xl p-5 flex flex-col justify-center items-center mb-4">
                                    {isPrimitive !== null ? (
                                        <div className="text-center w-full">
                                            <div className="text-xs text-slate-400 mb-2 font-mono">Primitive Root Check</div>
                                            <div className={`text-4xl font-black tracking-wider mb-2 ${isPrimitive ? 'text-green-300' : 'text-red-300'}`}>
                                                {isPrimitive ? "YES" : "NO"}
                                            </div>
                                            <div className="text-sm font-mono text-slate-300">
                                                {a} {isPrimitive ? "LÀ" : "KHÔNG LÀ"} căn nguyên thủy của {n}
                                            </div>
                                        </div>
                                    ) : (
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                    <div className="bg-white/5 rounded-lg p-3">
                                        <div className="text-blue-300 font-bold mb-1">φ(n)</div>
...
export default PrimitiveRootPage;
